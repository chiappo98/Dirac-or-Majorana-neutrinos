...
The standard Higgs mechanism of the generation of fermion masses proceed through the Lagrangian 
\begin{equation}
\mathcal{L}_Y(x)=-\sqrt{2}\sum_{l_1,l_2}\bar{\psi}_{l_1L}(x)Y_{l_1l_2}l'_{2R}(x)\phi(c)+h.c.
\end{equation}
where
\begin{equation}
\psi_{lL}(x)=\begin{bmatrix}\nu'_{lL}(x)\\ l'_L(x) \end{bmatrix}\quad(l=e, \mu, \tau),\qquad
\phi=\begin{bmatrix}\phi_+(x)\\ \phi_0(x) \end{bmatrix}
\end{equation}
are lepton and Higgs doublets, while $l'_R(x)$ is a right handed lepton singlet field and $Y$ is a $3\times3$ dimensionless complex matrix containing coupling constants %of yukawa interaction??
