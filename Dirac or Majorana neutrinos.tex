...
\mathcal{L}_Y^{\nu_e}(x)=-\frac{g_{\nu_e}}{\sqrt{2}}v(\bar{\nu}_{eL}\nu_{eR}+\bar{\nu}_{eR}\nu_{eL})-\frac{g_{\nu_e}}{\sqrt{2}}(\bar{\nu}_{eL}\nu_{eR}+\bar{\nu}_{eR}\nu_{eL})H(x)
\end{equation}
so that finally we get a (Dirac) mass for the neutrino, setting $g_{\nu_e}v/\sqrt{2}=m_{\nu_e}$
\begin{equation}
\mathcal{L}_{mass}^{\nu_e}=-m_{\nu_e}\bar{\nu}_{eL}\nu_{eR}+h.c.
\end{equation}
...
\end{equation}
where $M_l$ and $M_h$ are $3\times3$ and $m\times m$ real diagonal matrices. If the mass eigenvalues of $M_R$ are much higher than the scale of electroweak symmetry breaking \emph{v}, we obtain three light neutrinos $\nu_l$ and \emph{m} heavy neutrinos, \emph{N}.
\begin{equation}
