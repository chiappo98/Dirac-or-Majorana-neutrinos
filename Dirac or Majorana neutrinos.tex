...
Since we are dealing only with left-handed neutrino field $\nu_{L}$ instead of the lepton doublet $\psi_{L}$, it is not possible to generate Majorana masses through the usual coupling with the Higgs field and the subsequent symmetry breaking. 
...
The parameter $\Lambda$ is important since it represents some heavy new physics which has been integrated out, such as the exchange of a very heavy Majorana sterile neutrino.
...
If we assume the mass of the heaviest neutrino (mass) field to be $m_3\simeq5\times10^{-2}\SI{}{eV}$ and $x_3\leq1$ (although parameters $x_i$ are unknown) we obtain the estimate $\Lambda\leq10^{15}\SI{}{GeV}$~\cite{ref:Bil}.
