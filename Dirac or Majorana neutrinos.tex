...
Fermi called this particle \emph{neutrino}, and it was finally discovered in 1956 by Cowan and Reines at the Savannah River nuclear reactor in South Carolina, where they set up a large tank of water and watched for inverse beta decay reactions (see section \ref{sec:prod}).
...
\begin{equation} 
\psi=\begin{bmatrix}\psi_1\\ \psi_2 \\ \psi_3 \\ \psi_4 \end{bmatrix}=\begin{bmatrix}\psi_L\\ \psi_R \end{bmatrix}
\end{equation}
...
\end{equation}
where $u_L$ and $u_R$ are two component spinors. For simplicity lets choose in a frame in which a particle is at rest, i.e. $\vec{p}=0$. Since Dirac equation must satisfy special relativity we have that $E^2=p^2+m^2$ and therefore $E=\pm m$. We thus found positive energy solutions
%\begin{equation}
